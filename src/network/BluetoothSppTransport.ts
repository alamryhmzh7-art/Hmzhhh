...
* 
* Diagnostic decoding (OBD-II / ISO-TP / UDS) must remain outside
...
console.log(
'[BUILD-ID] BT-TRANSPORT-AUDITED-V3-20260912'
);
...
* 
* Bluetooth packets are not guaranteed to arrive in complete
* application frames, therefore incoming data must be accumulated.
  */
...
* 
* Keep every listener handle so disconnect() can remove it.
  */
...
* Application listeners.
  */
  ...
  Array<(state: ConnectionStatus, error?: string) => void> = [];

private dataListeners:
Array<(data: Uint8Array) => void> = [];

private canFrameListeners:
Array<(frame: CanFrame) => void> = [];

private klinePacketListeners:
Array<(pkt: DecodedBinaryPacket) => void> = [];

/**

* One outstanding request of each response type is supported by
* this transport layer.
* 
* Higher-level TransportManager should serialize diagnostic
...
private pingTimeoutHandle: ReturnType<typeof setTimeout> | null = null;
...
public updateConfig(config: ConnectionConfig): void {
this.config = config;
}
...
/**
 * Never carry an incomplete frame from an old connection
...
try {
  ...
  if (!isNative) {
    ...
    /**
     * Classic Bluetooth SPP is not the same thing as
     * Web Bluetooth GATT.
     *
     * Web Bluetooth therefore must not pretend to provide
     * native Bluetooth Classic SPP.
     *
     * Web Serial is kept as a development/test fallback.
     */
    if (
      ...
      throw new Error(
        'Native Bluetooth Classic SPP is required on Android. Web Serial is available only where the browser supports it.'
      );
    }
  ...
  const targetMac =
    (this.config.bluetoothMacAddress || '')
      .trim()
      .toUpperCase();
...
const devicesMap =
  new Map<string, BluetoothDeviceInfo>();
...
try {
  ...
  /**
   * Previously this function inserted hard-coded Bluetooth
   * addresses into the real device list.
   *
   * That is incorrect because the UI cannot distinguish
   * discovered hardware from fabricated entries.
   *
   * Only actual saved/paired/discovered devices are returned.
   */
  ...
  if (!isNative) {
    ...
    /**
     * Do NOT claim Web Bluetooth Classic SPP support.
     *
     * Web Bluetooth is GATT-based and does not provide
     * Android-style RFCOMM/SPP access to the ESP32
     * BluetoothSerial socket.
     */
  ...
  try {
    ...
    const pairedResult =
      await BluetoothSpp.getPairedDevices();
  ...
  try {
    ...
    foundHandle =
      await BluetoothSpp.addListener(
...
try {
  this.nativeDisconnectListener?.remove();
} catch {}

this.nativeDataListener = null;
this.nativeDisconnectListener = null;
...
private clearPendingResolvers(): void {
...
if (this.canStatusTimeoutHandle) {
  clearTimeout(
    this.canStatusTimeoutHandle
  );
}

if (this.klineInitTimeoutHandle) {
  clearTimeout(
    this.klineInitTimeoutHandle
  );
}

if (this.klineStatusTimeoutHandle) {
  clearTimeout(
    this.klineStatusTimeoutHandle
  );
}

if (this.klineFrameTimeoutHandle) {
  clearTimeout(
...
this.canStatusTimeoutHandle = null;
this.klineInitTimeoutHandle = null;
this.klineStatusTimeoutHandle = null;
this.klineFrameTimeoutHandle = null;
...
console.log(
  `[BT-CAN-TX] ID=0x${canId.toString(16).toUpperCase()} DLC=${data.length} DATA=[${hexData}]`
);
...
if (typeof data === 'string') {
  ...
  console.warn(
    '[BT-RX] String data received; converting byte-by-byte'
  );
 ...
 */
for (
  const listener of [...this.dataListeners]
) {
  try {
    listener(newBytes);
  } catch (error) {
...
/**
 * Append incoming bytes to the stream buffer.
 */
const merged =
  new Uint8Array(
...
/**
 * BinaryProtocol is responsible for finding complete
 * AA55 frames and returning incomplete bytes as remainingBuffer.
 */
const parsed =
...
for (
  ...
  console.log(
    `[BT-RX-FRAME] CMD=0x${packet.cmd.toString(16).toUpperCase()} LEN=${packet.payload.length} HEX=[${pktHex}]`
  );
...
/**
 * ASCII fallback is intentionally kept only for legacy
 * ELM-style adapters. It must not interfere with valid
 * binary packets.
 */
if (
...
// ---------------------------------------------------------------------------
// LEGACY ASCII ELM FALLBACK
// ---------------------------------------------------------------------------

private checkAndParseAsciiLines(): void {
...
for (
  ...
  /**
   * This is only an ELM-compatible OBD response
   * heuristic. It must not be treated as a general
   * CAN-frame decoder.
   */
  const modeByte =
    bytes[0];
...
private async startNativeBtReadLoop(): Promise<void> {

/**
 * Remove previous listeners before creating new ones.
 * This prevents duplicated CAN frames after reconnects.
 */
try {
  this.nativeDataListener?.remove();
} catch {}
...
this.nativeDisconnectListener =
  await (
    ...
    () => {
      ...
      /**
       * Do not recursively call the complete public
       * disconnect path from inside the native listener
...
private async handleNativeDisconnect(): Promise<void> {
